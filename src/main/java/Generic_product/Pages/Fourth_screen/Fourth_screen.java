...
public class Fourth_screen extends Generic_BasePage {
    ...
    public boolean isOnFourthScreen() {
        try {
            WebElement element = longwait.until(ExpectedConditions.visibilityOfElementLocated(verificationCodeFirstDigitInput));
    ...
    public boolean isTimerTextAndCountingDown(int maxWaitSeconds) {
        try {
            WebElement container = customWait(2).until(ExpectedConditions.visibilityOfElementLocated(containerLocator));
            System.out.println("Debug: Timer container found and visible.");

            WebElement textSpan = customWait(2).until(ExpectedConditions.visibilityOf(container.findElement(textSpanLocator)));
            ...
            WebElement timerElement = customWait(2).until(ExpectedConditions.visibilityOf(container.findElement(timerLocator)));
    ...
    public WebElement getVerificationCodeSentTextElement() {
        return customWait(2).until(ExpectedConditions.visibilityOfElementLocated(verificationCodeSentTextLocator));
    }

    public boolean isVerificationCodeTextDisplayedImmediately() {
        try {
            customWait(2).until(ExpectedConditions.visibilityOfElementLocated(verificationCodeSentTextLocator));
    ...
    public boolean areResendAndCallButtonsDisabledImmediately() {
        try {
            WebElement resendButtonElement = customWait(5).until(ExpectedConditions.visibilityOfElementLocated(resendCodeButton));
            WebElement callButtonElement = customWait(5).until(ExpectedConditions.visibilityOfElementLocated(callMeButton));
    ...
    public boolean isTopImageDisplayedAndCorrect() {
        try {
            WebElement imageElement = customWait(5).until(ExpectedConditions.visibilityOfElementLocated(topImage));
    ...
    public boolean isImageLoaded(By imageLocator) {
        try {
            WebElement image = customWait(5).until(ExpectedConditions.visibilityOfElementLocated(imageLocator));
